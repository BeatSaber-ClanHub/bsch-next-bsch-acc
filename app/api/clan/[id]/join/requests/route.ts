...
export const GET = async (
  ...
  try {
    ...
    const orderByFields = ["name", "createdAt"] as const;
    ...
    const [requestsError, requests] = await getPendingJoinRequests({
      clanId: clan.id,
      queryConfig: queryParams,
    });
    if (requestsError) throw requestsError;

    const [countError, count] = await countPendingJoinRequests({
      clanId: clan.id,
