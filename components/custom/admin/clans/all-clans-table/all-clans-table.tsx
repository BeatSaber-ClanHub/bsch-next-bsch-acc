...
import { columns } from "@/components/custom/admin/clans/all-clans-table/columns";
...
export function AllClansTable({ role }: { role: Role }) {
  // State
  ...
  async function getClans({
    ...
    const response = await fetch(
      ...
      }&includeHiddenClans=true&limit=${pageSize}&orderBy=role&sortDirection=asc${
  ...
  const { data, status, isFetching } = useQuery({
    queryKey: ["all_clans", pageIndex, pageSize, { search: debouncedValue }],
  ...
  if (status === "error") {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>There was an issue fetching clans!</AlertDescription>
      </Alert>
    );
  }

  let pageCount = 0;
  let searchResults: ClanWithClanOwnerInfoAndBasicData[] = [];
...
interface DataTableProps {
  columns: ColumnDef<ClanWithClanOwnerInfoAndBasicData>[];
  data: ClanWithClanOwnerInfoAndBasicData[];
  pageCount: number;
