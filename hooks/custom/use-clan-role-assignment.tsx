...
interface RoleAssignmentRequest {
  userId: string;
  newRole: ClanStaffRole;
  callback?: (props: RoleAssignmentRequest) => unknown | undefined;
}

const useClanRoleAssignment = (clanId: string) => {
  ...
  >({
    mutationFn: async ({ newRole, userId }: RoleAssignmentRequest) => {
      const response = await fetch(
        `/api/clan/${clanId}/member/${userId}/role/assign`,
        {
          method: "POST",
          headers: {
...
export default useClanRoleAssignment;
